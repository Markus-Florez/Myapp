...
class CrearDiagElecActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        enableEdgeToEdge()
        setContentView(R.layout.activity_crear_diag_elec)
        ...
        fun instantUpload(target: String, fileName: String, uri: Uri?) {
            ...
            val storageBase = Firebase.storage.reference
                .child("diag_elec").child(sanitizeKey(marca)).child(sanitizeKey(modelo)).child(sanitizeKey(anio)).child("meta")
            val ref = storageBase.child(fileName)
            ref.putFile(uri).addOnSuccessListener {
                ref.downloadUrl.addOnSuccessListener { u ->
                    Firebase.database.reference
                        .child("diag_elec").child(sanitizeKey(marca)).child(sanitizeKey(modelo)).child(sanitizeKey(anio))
                        .child(target).setValue(u.toString()).addOnSuccessListener {
        ...
        btnGuardar.setOnClickListener {
            ...
            fun upload(uri: Uri?, name: String, onUrl: (String) -> Unit, onDone: () -> Unit) {
                ...
                ref.putFile(uri).addOnSuccessListener { ref.downloadUrl.addOnSuccessListener { u -> onUrl(u.toString()); onDone() } }
                    .addOnFailureListener { onUrl(""); onDone() }
            }
            var urlMarca = ""
            var urlModelo = ""
            var urlAnio = ""
            ...
            fun stepDone() { pending -= 1; if (pending == 0) {
                ...
                val ref = Firebase.database.reference
                    .child("diag_elec").child(sanitizeKey(marca)).child(sanitizeKey(modelo)).child(sanitizeKey(anio))
                ref.setValue(data).addOnSuccessListener { Toast.makeText(this, "Datos guardados", Toast.LENGTH_SHORT).show() }
            }}
        ...
        btnCrearCarpetas.setOnClickListener {
            ...
            intent.putExtra("baseNode", "diag_elec")
    ...
    private fun showPickMarcaInline(inputMarca: EditText, imgMarca: ImageView, title: android.widget.TextView, list: android.widget.ListView) {
        ...
        ref.get().addOnSuccessListener { snap ->
            ...
            if (marcas.isEmpty()) {
                Toast.makeText(this, "No hay marcas creadas", Toast.LENGTH_SHORT).show(); return@addOnSuccessListener
            }
            title.text = "Marcas"
    ...
    private fun showPickModeloInline(inputMarca: EditText, inputModelo: EditText, imgModelo: ImageView, title: android.widget.TextView, list: android.widget.ListView) {
        ...
        title.text = "Cargando..."
        if (marca.isBlank()) {
            val refAll = Firebase.database.reference.child("diag_elec")
        ...
        val ref = Firebase.database.reference.child("diag_elec").child(sanitizeKey(marca))
        ref.get().addOnSuccessListener { snap ->
    ...
    private fun showPickAnioInline(inputMarca: EditText, inputModelo: EditText, inputAnio: EditText, imgAnio: ImageView, title: android.widget.TextView, list: android.widget.ListView) {
        ...
        title.text = "Cargando..."
        val ref = Firebase.database.reference.child("diag_elec").child(sanitizeKey(marca)).child(sanitizeKey(modelo))
        ref.get().addOnSuccessListener { snap ->
