...
class VerProcedimientosActivity : ComponentActivity() {
    ...
    private fun loadFiles(marca: String, modelo: String, anio: String, folder: String) {
        ...
        val folderKey = sanitizeKey(folder)
        val ref = Firebase.database.reference
            .child("diag_mcos").child(marca).child(modelo).child(anio)
            .child("folders").child(folderKey).child("files")
        ref.get().addOnSuccessListener { snap ->
            val items = mutableListOf<Item>()
    ...
    private fun openFile(item: Item) {
        ...
        if (type.startsWith("image/")) {
            ...
            runCatching {
                ...
                Toast.makeText(this, "No hay app para abrir", Toast.LENGTH_SHORT).show()
            }
        }
    }
                    ...
                    runCatching {
                        ...
                        conn.inputStream.use {
                            ...
                            if (bmp != null) {
                                ...
                                this@VerProcedimientosActivity.runOnUiThread { img.setImageBitmap(bmp) }
                            }
                        }
                        conn.disconnect()
                    }
